...
2. **`lessons/ai-prediction-lab/`(AI는 어떤 문제를 풀 수 있을까?)** — 공부 시간 하나로 시험 점수를 예측하는 회귀(활동 1, 최소제곱 회귀)부터 시작해, 관계없는 정보(사장님 기분·지나가는 고양이 수)까지 섞인 기온·유동인구 데이터로 "붐빔/한산"을 가르는 분류(활동 2, k-최근접 이웃)를 거쳐, 마지막엔 정답 라벨이 아예 없는 도시 기후 데이터를 AI가 스스로 비슷한 것끼리 묶는 군집(활동 3, k-평균)까지 세 가지 기계학습 방식을 체험합니다. 각 활동을 마치면 그 방식(회귀/분류/군집)이 무엇인지 간단한 개념 설명이 함께 나옵니다.
