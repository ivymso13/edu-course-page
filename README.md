...
활동지 목록 페이지는 `units/ai-search/`이고, 강 건너기와 8-퍼즐로 상태·간선과 맹목적 탐색을 익힌 뒤 같은 학교 지도에서 균일 비용 탐색과 A* 탐색을 비교하는 활동 네 개가 이어집니다.

1. **`lessons/search-bfs-delivery/`(문제 해결과 탐색)** — 늑대·양·양배추 강 건너기를 직접 조작하며 상태·행동·초기 상태·목표 상태 개념을 익힙니다.
2. **`lessons/search-eight-puzzle/`(8-퍼즐로 보는 맹목적 탐색)** — 8-퍼즐을 직접 맞춘 뒤 너비 우선 탐색이 가까운 상태부터 층별로 넓히고 중복 상태를 제외하며 최소 이동 경로를 찾는 과정을 관찰합니다.
3. **`lessons/search-cost-delivery/`(균일 비용 탐색으로 가장 빠른 길 찾기)** — 장소마다 이동 비용이 다를 때, 균일 비용 탐색이 지금까지 든 비용 g(n)이 가장 작은 곳부터 오픈 리스트로 넓혀가며 실제로 가장 빠른 길을 찾는다는 것을 체험합니다.
4. **`lessons/search-astar-delivery/`(휴리스틱값으로 익히는 A* 탐색)** — 3단계와 같은 학교 지도에서 A* 탐색이 균일 비용 탐색보다 적은 상태만 확인하고도 같은 길을 찾는 것을 확인한 뒤, 8-퍼즐 새 문제에서 g(n), h(n), f(n)을 비교하며 다음 상태를 선택하는 원리를 체험합니다.

## 활동지 04 · 탐색의 다양한 문제 해결 사례

활동지 목록 페이지는 `units/search-problem-cases/`이고, 상대가 있는 게임에서 컴퓨터가 Min-Max 알고리즘으로 수를 고르는 과정을 체험하는 활동 두 개가 이어집니다.

1. **`lessons/search-tictactoe-minimax/`(게임 트리 예시 1)** — Min-Max 알고리즘을 적용한 컴퓨터와 틱택토를 하고, 게임이 끝난 뒤 컴퓨터가 상대의 최선 대응까지 살펴 수를 고른 과정을 확인합니다. 각 수 아래에는 후보 수·MAX/MIN 차례·말단 점수(+1/0/-1)·값이 아래에서 위로 전달되는 과정·최종 선택 간선을 보여주는 Min-Max 의사결정 트리가 있습니다(전체 게임 트리 대신 지금 판단에 필요한 대표 가지만 표시하고, 생략된 부분은 안내합니다).
2. **`lessons/search-othello-minimax/`(게임 트리 예시 2)** — 같은 Min-Max 알고리즘을 적용한 컴퓨터와 4×4 오델로를 두며, 상대의 돌을 뒤집는 규칙과 둘 수 있는 자리가 없을 때의 패스 규칙을 익힙니다. 활동 01과 동일하게 게임이 끝난 뒤 각 수 아래에서 후보 수·MAX/MIN 차례·돌 개수로 정해지는 말단 점수(+1/0/-1)·값 전달·최종 선택을 의사결정 트리로 확인할 수 있고, 상대가 패스한 지점은 트리에서 별도 패스 노드로 표시됩니다.

## 활동지 05 · 기계학습과 데이터

1. **`lessons/ai-problem-method/`(어떤 방법으로 해결할까?)** — 규칙과 계산 또는 여러 사례에서 학습하는 방법 중 문제에 더 알맞은 해결 방식을 선택하는 도입 활동입니다.
2. **`lessons/ai-prediction-lab/`(AI는 어떻게 값을 예측할까?)** — 공부 시간 하나로 시험 점수를 예측하는 것부터 시작해, 기온·강수량 두 가지, 마지막엔 관계없는 정보(화분 개수·직원 이름 글자 수)까지 섞인 다섯 가지 정보로 값을 예측합니다. 학생이 먼저 값을 예측한 뒤 최소제곱 회귀로 계산한 AI의 예측과 비교하고, 활동 2·3에서는 슬라이더로 입력값을 직접 바꿔가며 예측이 실시간으로 달라지는 과정을 관찰합니다. 활동 3에서는 관계없는 정보를 바꿔도 예측이 전혀 변하지 않는다는 것과, AI의 예측이 실제값과 정확히 일치하지 않을 수 있다는 것(오차)을 직접 확인합니다.

## 활동지 06 · 기계학습 알고리즘

같은 마트 과일 선별 AI를 소재로 세 활동이 이어집니다. 활동지 목록 페이지는 `units/machine-learning-algorithms/`이고, 각 활동은 `lessons/` 아래의 기존 독립 URL을 유지합니다.
